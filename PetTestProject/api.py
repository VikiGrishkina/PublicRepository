...
class PetFriends:
    ...
    def add_pet_photo(self, auth_key: json, pet_id: str, pet_photo: str) -> json:
        """Метод отправляет (постит) на сервер фото для уже существующего питомца и возвращает статус
    ...
    def add_pet_simple(self, auth_key: json, name: str, animal_type: str,
                    age: int) -> json:
        """Метод отправляет (постит) на сервер данные питомца без фото и возвращает статус
        запроса на сервер и результат в формате JSON с данными питомца"""
        data = MultipartEncoder(
            fields={
                'name': name,
